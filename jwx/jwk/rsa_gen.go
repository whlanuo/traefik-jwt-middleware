...
type rsaPrivateKeyMarshalProxy struct {
	XkeyType                jwa2.KeyType      `json:"kty"`
...
func (h rsaPrivateKey) KeyType() jwa2.KeyType {
	return jwa2.RSA
}
...
func (h *rsaPrivateKey) iterate(ctx context.Context, ch chan *HeaderPair) {
	...
	pairs = append(pairs, &HeaderPair{Key: "kty", Value: jwa2.RSA})
	if h.algorithm != nil {
...
func (h *rsaPrivateKey) UnmarshalJSON(buf []byte) error {
	...
	if err := json2.Unmarshal(buf, &proxy); err != nil {
		return errors.Wrap(err, `failed to unmarshal rsaPrivateKey`)
	}
	if proxy.XkeyType != jwa2.RSA {
	...
	if h.d = nil; proxy.Xd != nil {
		decoded, err := base642.DecodeString(*(proxy.Xd))
		if err != nil {
	...
	if h.dp = nil; proxy.Xdp != nil {
		decoded, err := base642.DecodeString(*(proxy.Xdp))
		if err != nil {
	...
	if h.dq = nil; proxy.Xdq != nil {
		decoded, err := base642.DecodeString(*(proxy.Xdq))
		if err != nil {
	...
	if h.e = nil; proxy.Xe != nil {
		decoded, err := base642.DecodeString(*(proxy.Xe))
		if err != nil {
	...
	if h.n = nil; proxy.Xn != nil {
		decoded, err := base642.DecodeString(*(proxy.Xn))
		if err != nil {
	...
	if h.p = nil; proxy.Xp != nil {
		decoded, err := base642.DecodeString(*(proxy.Xp))
		if err != nil {
	...
	if h.q = nil; proxy.Xq != nil {
		decoded, err := base642.DecodeString(*(proxy.Xq))
		if err != nil {
	...
	if h.qi = nil; proxy.Xqi != nil {
		decoded, err := base642.DecodeString(*(proxy.Xqi))
		if err != nil {
...
func (h rsaPrivateKey) MarshalJSON() ([]byte, error) {
	var proxy rsaPrivateKeyMarshalProxy
	proxy.XkeyType = jwa2.RSA
	proxy.Xalgorithm = h.algorithm
	if len(h.d) > 0 {
		v := base642.EncodeToString(h.d)
		proxy.Xd = &v
	}
	if len(h.dp) > 0 {
		v := base642.EncodeToString(h.dp)
		proxy.Xdp = &v
	}
	if len(h.dq) > 0 {
		v := base642.EncodeToString(h.dq)
		proxy.Xdq = &v
	}
	if len(h.e) > 0 {
		v := base642.EncodeToString(h.e)
		proxy.Xe = &v
	}
	...
	if len(h.n) > 0 {
		v := base642.EncodeToString(h.n)
		proxy.Xn = &v
	}
	if len(h.p) > 0 {
		v := base642.EncodeToString(h.p)
		proxy.Xp = &v
	}
	if len(h.q) > 0 {
		v := base642.EncodeToString(h.q)
		proxy.Xq = &v
	}
	if len(h.qi) > 0 {
		v := base642.EncodeToString(h.qi)
		proxy.Xqi = &v
	}
...
func (h rsaPublicKey) KeyType() jwa2.KeyType {
	return jwa2.RSA
}
