...
func ParseKey(data []byte) (Key, error) {
	var hint struct {
		Kty string           `json:"kty"`
		D   json2.RawMessage `json:"d"`
	}

	if err := json2.Unmarshal(data, &hint); err != nil {
		return nil, errors.Wrap(err, `failed to unmarshal JSON into key hint`)
	}

	var key Key
	switch jwa2.KeyType(hint.Kty) {
	case jwa2.RSA:
		if len(hint.D) > 0 {
	...
	if err := json2.Unmarshal(data, key); err != nil {
...
func (s *Set) UnmarshalJSON(data []byte) error {
	var proxy struct {
		Keys []json2.RawMessage `json:"keys"`
	}

	if err := json2.Unmarshal(data, &proxy); err != nil {
...
func Parse(in io.Reader) (*Set, error) {
	var s Set
	if err := json2.NewDecoder(in).Decode(&s); err != nil {
