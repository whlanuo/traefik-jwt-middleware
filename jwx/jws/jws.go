...
import (
	...
	verify2 "github.com/whlanuo/traefik-jwt-middleware/jwx/jws/verify"
	"io"
	"strings"
	"unicode"
...
func (s *payloadSigner) Algorithm() jwa2.SignatureAlgorithm {
...
func Sign(payload []byte, alg jwa2.SignatureAlgorithm, key interface{}, options ...Option) ([]byte, error) {
	var hdrs Headers
	...
	if jwkKey, ok := key.(jwk2.Key); ok {
		...
		if kid := jwkKey.KeyID(); kid != "" {
			if err := hdrs.Set(jwk2.KeyIDKey, kid); err != nil {
				return nil, errors.Wrap(err, `set key ID from jwk.Key`)
			}
		}
	}

	signer, err := sign2.New(alg)
	if err != nil {
	...
	hdrbuf, err := json2.Marshal(hdrs)
	if err != nil {
		return nil, errors.Wrap(err, `failed to marshal headers`)
	}

	buf := pool2.GetBytesBuffer()
	defer pool2.ReleaseBytesBuffer(buf)
...
// multiple signers.
//
func SignLiteral(payload []byte, alg jwa2.SignatureAlgorithm, key interface{}, headers []byte) ([]byte, error) {
	signer, err := sign2.New(alg)
	if err != nil {
		return nil, errors.Wrap(err, `failed to create signer`)
	}
...
func SignMulti(payload []byte, options ...Option) ([]byte, error) {
	...
	for _, signer := range signers {
		...
		hdrbuf, err := json2.Marshal(protected)
		if err != nil {
			return nil, errors.Wrap(err, `failed to marshal headers`)
		}
...
func Verify(buf []byte, alg jwa2.SignatureAlgorithm, key interface{}) (ret []byte, err error) {
	verifier, err := verify2.New(alg)
	if err != nil {
	...
	verifyBuf := pool2.GetBytesBuffer()
	defer pool2.ReleaseBytesBuffer(verifyBuf)
...
func VerifyWithJKUAndContext(ctx context.Context, buf []byte, jwkurl string, options ...Option) ([]byte, error) {
	key, err := jwk2.FetchHTTPWithContext(ctx, jwkurl, options...)
	if err != nil {
...
func VerifyWithJWK(buf []byte, key jwk2.Key) (payload []byte, err error) {
	var rawkey interface{}
	if err := key.Raw(&rawkey); err != nil {
		return nil, errors.Wrap(err, `failed to materialize jwk.Key`)
	}

	payload, err = Verify(buf, jwa2.SignatureAlgorithm(key.Algorithm()), rawkey)
	if err != nil {
		return nil, errors.Wrap(err, "failed to verify message")
	}
	return payload, nil
...
func VerifyWithJWKSet(buf []byte, keyset *jwk2.Set, keyaccept JWKAcceptFunc) ([]byte, error) {
	if keyaccept == nil {
...
type fullMessageProxy struct {
	...
	Signature json2.RawMessage `json:"signature"`
	Headers   json2.RawMessage `json:"header"` // あれ、"s"いらないんだっけ
	Protected json2.RawMessage `json:"protected"`
...
func (proxy *fullMessageProxy) encodedSignature() (*encodedSignature, error) {
	...
	if err := json2.Unmarshal(proxy.Protected, &encodedSig.Protected); err != nil {
		return nil, errors.Wrap(err, `failed to unmarshal 'protected' field`)
	}
	if err := json2.Unmarshal(proxy.Signature, &encodedSig.Signature); err != nil {
		return nil, errors.Wrap(err, `failed to unmarshal 'signature' field`)
	}
	h := NewHeaders()
	if err := json2.Unmarshal(proxy.Headers, h); err != nil {
...
func parseJSON(src io.Reader) (result *Message, err error) {
	var proxy fullMessageProxy
	if err := json2.NewDecoder(src).Decode(&proxy); err != nil {
		return nil, errors.Wrap(err, `failed to unmarshal jws message`)
	}
	...
	for i, sig := range proxy.Signatures {
		...
		if l := len(sig.Protected); l > 0 {
			...
			if err := json2.Unmarshal(hdrbuf, &plainSig.protected); err != nil {
...
func parseCompact(rdr io.Reader) (m *Message, err error) {
	...
	var hdr stdHeaders
	if err := json2.Unmarshal(decodedHeader, &hdr); err != nil {
